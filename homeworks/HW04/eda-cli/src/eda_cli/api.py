...
@app.post(
    ...
    tags=["quality-flags"],
    summary="Получение флагов качества по CSV-файлу"
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityFlagsResponse:
    """
    Эндпоинт, который принимает CSV-файл, вычисляет флаги качеста дадасета
    и возвращает их
    """
    ...
    return QualityFlagsResponse(flags=QualityFlags(**quality_flags))
