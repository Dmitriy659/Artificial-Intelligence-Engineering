...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    # категориальные признаки с большим числом уникальныъ значений
    high_card_threshold = int(len(summary.columns) * 0.8)
    high_card_cols = [
        col.name
        for col in summary.columns
...
def get_problematic_cols(missing_df: pd.DataFrame, min_missing_share: float) -> list[str]:
    """
    Получение списка с проблемными столбцами
    """
    if missing_df.empty:
        return []
