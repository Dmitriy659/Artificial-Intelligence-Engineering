...
from .core import (
    ...
    missing_table,
    summarize_dataset,
    top_categories, get_problematic_cols,
)
from .viz import (
...
def report(
    ...
    problematic_cols = get_problematic_cols(missing_df, min_missing_share)
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        if quality_flags["has_constant_columns"]:
            f.write(f"- Конастантные столбцы: **{', '.join(quality_flags['constant_columns'])}**\n\n")

        f.write(f"- Наличие категориальных признаков с кол-вом уникальных значений >80%: **{quality_flags['has_high_cardinality_categoricals']}**\n\n")
        if quality_flags["has_high_cardinality_categoricals"]:
            f.write(f"- Столбцы с бошьим кол-вом уникальных значений: **{', '.join(quality_flags['high_cardinality_columns'])}**\n\n")
        ...
        f.write("## Проблемные колонки (по порогу пропусков)\n\n")
        f.write(f"Порог `min_missing_share`: **{min_missing_share:.2f}**\n\n")

        if missing_df.empty:
            f.write("Датасет пуст, пропуски не анализировались.\n\n")
        elif not problematic_cols:
            f.write("Нет колонок, превышающих заданный порог пропусков.\n\n")
        else:
            f.write("Колонки, где доля пропусков превышает порог:\n\n")
